...
include_once('WpReferrals_InstallIndicator.php');

class WpReferrals_LifeCycle extends WpReferrals_InstallIndicator {
    ...
    public function displayRefUrl ($actionName) {
        //return admin_url('admin-ajax.php') . '?action=' . $actionName;
        $me = wp_get_current_user();

            ?>
			<h2><?php _e('Your affiliate URL', 'wp-referrals'); ?></h2>
            <p><input type="text" value="<?php echo bloginfo('home')."?ref=".$me->ID; ?>" /></p>
            <?php  
    }
	
	
    public function addRefVarsFilter ($vars) {
         $vars[] = "ref";
         return $vars;
    }
	
    public function setRefCookie () {

    	if ( !is_admin() && !isset($_COOKIE['ref_newvisitor'])) {
		setcookie( 'ref_newvisitor', $_GET['ref'], time()+3600*24*30, COOKIEPATH, COOKIE_DOMAIN, false, false);
	    }


    }	
	
	    public function saveRefID ($user_id) {

    	if ( isset($_COOKIE['ref_newvisitor'])) {
        update_user_meta($user_id, 'ref_id', $_COOKIE['ref_newvisitor']);
	    }


    }	                                                                       

		public function saveRefCommission ($order_id) {

    	if ( $order = new WC_Order( $order_id ) ) {
        // Get the order from the id & the info we need
        $order_total = $order->get_total();
		$customer = wp_get_current_user();
		$refovod = get_user_meta($customer->ID,'ref_id', 1);
		$com_value = get_option('WpReferrals_Plugin_Commission');
		$ref_comission = $order_total*$com_value/100;
		
		if($ref_funds = get_user_meta( $refovod, 'account_funds', 1)) {

		$ref_earnings_all = $ref_funds + $ref_comission;

			
		update_user_meta( $refovod, 'account_funds', $ref_earnings_all, $ref_funds );	
		} else {
			
			add_user_meta( $refovod, 'account_funds', $ref_comission );
		}
